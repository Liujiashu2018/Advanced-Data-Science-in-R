...
n_flights<-
  ...
  mutate(num_departing_flights=case_when(
    cancelled == 0 ~ 1, 
    cancelled > 0 ~ 0))%>% 
  group_by(name, year, month) %>% 
  summarise(num_flights = sum(num_departing_flights),
...
```{r}
show_query(n_flights)
```

```{sql connection=con_air}
SELECT `name`, `year`, `month`, SUM(`num_departing_flights`) AS `num_flights`, COUNT(DISTINCT `dest`) AS `num_dest`, AVG(`air_time`) AS `avg_length`, AVG(`distance`) AS `avg_distance`, AVG(`arr_delay` > 20.0) AS `prop_late_over20`
...
[Jiashu's Shiny App](https://jiashu-liu.shinyapps.io/intro_to_shiny-master/)
