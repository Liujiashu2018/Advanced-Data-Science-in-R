...
rf_explain<-
  explain_tidymodels(
    ...
    y = lending_training %>% mutate(Class_num = as.integer(Class =="good")) %>% 
        pull(Class_num),
    label = "rf"
  )
rf_explain
...
```{r}
cp_profile<-function(explainer, vname, new_ob){
  rf_cpp<-predict_profile(explainer = rf_explain, 
                             variables = vname, 
                             new_observation = new_ob) 

plot<-rf_cpp%>% 
  ggplot(aes(x =.data[[vname]],
             y = `_yhat_`) )+
  geom_line()
  return(plot)}
...
```{r}
rf_cp<-cp_profile(rf_explain, "int_rate", obs4)
rf_cp
```
...
rf_pdp <- model_profile(explainer = rf_explain, 
                        variables = c("int_rate","annual_inc", "open_il_24m"))
```
...
```{r}
new_obs <- lending_test %>% slice_head(n=3) 
new_obs
```
```{r}
#break-down plot
...
```{r}
plot(pp_rf)
#interpretation: The break-down profile shows that the average score is 0.848. The most important variable is open_il_24m. Therefore, having 0 installment accounts opened in past 24 months enhance to your average credit status by +0.029. 
```
```{r}
rf_bdp <- pp_rf %>% 
  as_tibble() %>% 
...
plot(rf_shap)# Interpretation: The result of the shap plot is pretty similar to the result of the break-down profile."open_il_24m" remains to be the most important variable, while verification_status seems to be the least important one. 
```
```{r}
#lime plot
set.seed(2)
...
```{r}
plot(lime_rf) +
  labs(x = "Variable") # The results of the lime plot are different from the results in the previous two graphs. In this plot, the number of personal finance inquiries "inq_fi" is an important variable in this plot. 
```
...
I want to try using the lasso model and the lime plot in econometric research to select a list of variables according to their importance. Especially when using a lime plot, I could see a variable's  different level of importance according to its range. 
...
```{r}
write_rds(rf_final, "final_model")
```
...
The one fooled me was the "Gun deaths in Florida" graph at the end of Week6 because the axis has been inverted. I think a good design of data visualization is a one which can accurately reflect researchers' findings. The graph could also be twisted in a way that the results are partially emphasized. The researchers could design the plot and, for example, exaggerate the difference between two items to make us believe what they want us to believe.   
